...
- **DNS name** — необязательное поле. Позволяет указать понятное имя сервиса,
  которое будет сохраняться вместе с туннелем.

Поле DNS можно оставить пустым. Все действия с туннелями доступны через
